...
public class NativeIO {
    public static class POSIX {
        ...
        public static void setCacheManipulator(
         ...
         * mlock.
         * This allows many tests to be run even when the operating system does
         * not
            ...
            if (NativeCodeLoader.isNativeCodeLoaded()) {
                try {
                    ...
                    cacheTimeout = conf.getLong(
                        ...
                        * 1000;
                    LOG.debug(
                        "Initialized cache for IDs to User/Group mapping with a "
                            + " cache timeout of " + cacheTimeout / 1000
                            + " seconds.");
                    ...
                    // after warning
                    PerformanceAdvisory.LOG
                        .debug("Unable to initialize NativeIO libraries", t);
                }
            }
        }
         ...
         */
        static void posixFadviseIfPossible(String identifier, FileDescriptor fd,
            ...
            if (!Shell.WINDOWS) {
                ...
                try {
                    ...
                    if (nioe.getErrorCode() == 6) {
                        ...
                        LOG.warn(
                            String.format("NativeIO.getFstat error (%d): %s",
                                nioe.getErrorCode(), nioe.getMessage()));
                        throw new NativeIOException("Unknown error",
                            Errno.UNKNOWN);
                    }
    ...
    static long getOperatingSystemPageSize() {
        try {
            ...
            LOG.warn(
                "Unable to get operating system page size.  Guessing 4096.", e);
            return 4096;
        }
    }
    ...
    public static FileInputStream getShareDeleteFileInputStream(File f,
        ...
        if (!Shell.WINDOWS) {
            ...
            // given offset.
            //
            FileDescriptor fd = NativeIO.Windows.createFile(f.getAbsolutePath(),
        ...
        if (!Shell.WINDOWS) {
            ...
            try {
                FileDescriptor fd = NativeIO.POSIX.open(
                    f.getAbsolutePath(), NativeIO.POSIX.O_WRONLY
                        | NativeIO.POSIX.O_CREAT | NativeIO.POSIX.O_EXCL,
                    permissions);
    ...
    private synchronized static void ensureInitialized() {
        if (!initialized) {
            cacheTimeout = new Configuration()
                .getLong("hadoop.security.uid.cache.secs", 4 * 60 * 60) * 1000;
     ...
     * FileChannle#transferTo already has its own batching of size 8 MB and 8
     * KB,
     * respectively.
     * In Windows Platform:
     ...
     * wrapper(Java_sun_nio_ch_FileChannelImpl_transferTo0)
     * used by FileChannel#transferTo for unbuffered IO is not implemented on
     * Windows.
     ...
     * the Storage#nativeCopyFileUnbuffered() function in hadoop-hdfs with
     * pre/post copy
     * checks.
     *
     * @param src
